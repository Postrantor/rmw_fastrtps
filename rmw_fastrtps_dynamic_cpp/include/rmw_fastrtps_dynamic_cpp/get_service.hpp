...
#ifndef RMW_FASTRTPS_DYNAMIC_CPP__GET_SERVICE_HPP_
#define RMW_FASTRTPS_DYNAMIC_CPP__GET_SERVICE_HPP_
...
/// 返回请求的原生 Fast DDS DataReader 句柄。 (Return a native Fast DDS DataReader handle for the
/// request.)
/**
 * 当服务句柄为 `NULL` 或服务句柄来自不同的 rmw 实现时，函数返回 `NULL`。
...
/// 返回响应的原生 Fast DDS DataWriter 句柄。 (Return a native Fast DDS DataWriter handle for the
/// response.)
/**
 * 当服务句柄为 `NULL` 或服务句柄来自不同的 rmw 实现时，函数返回 `NULL`。
 * (The function returns `NULL` when either the service handle is `NULL` or
