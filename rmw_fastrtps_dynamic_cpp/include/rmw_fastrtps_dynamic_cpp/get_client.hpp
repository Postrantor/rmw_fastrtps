...
/// 返回请求的原生 Fast DDS DataWriter 句柄。 (Return a native Fast DDS DataWriter handle for the
/// request.)
/**
...
/// 返回响应的原生 Fast DDS DataReader 句柄。 (Return a native Fast DDS DataReader handle for the
/// response.)
/**
 * 当客户端句柄为 `NULL` 或者客户端句柄来自不同的 rmw 实现时，函数返回 `NULL`。
 * (The function returns `NULL` when either the client handle is `NULL` or
